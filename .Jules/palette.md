...
**Learning:** React components using Lucide icons often omit `aria-label`s on icon-only buttons, making them completely opaque to screen readers. We found this pattern in both `TaskWidget.tsx` and `MessageBubble.tsx`.
**Action:** Always ensure any `<button>` containing only an icon includes a descriptive `aria-label` that clearly conveys the button's action to assistive technologies.
